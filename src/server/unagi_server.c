 ...
 */
typedef struct {
  cmdline_options_t opt;   /**< コマンドラインオプション */
...
static int connection_handle_put(request_t req, int so, server_t * sv) {
  if (sv->log_wp) {
    fprintf(sv->log_wp, "put label[%ld]=[%s] data[%ld]=[%s]\n",
 ...
 */
  ...
  while (1) {
    ...
    if (!send_bytes(so, occ.doc.label, occ.doc.label_len)) return 0;
    if (!send_bytes(so, " ", 1)) return 0;
    /* 出現位置を送信 */
    if (!send_num(so, occ.offset, ' ')) return 0;
    /* スニペット長 スニペット を送信 */
    if (!send_num(so, end - start, ' ')) return 0;
    if (!send_bytes(so, occ.doc.data + start, end - start)) return 0;
    if (!send_bytes(so, "\n", 1)) return 0;
  }
  if (cx != c) {
 ...
 */
  ...
  while (1) {
    ...
    if (!send_bytes(so, doc.label, doc.label_len)) return 0;
    if (!send_bytes(so, " ", 1)) return 0;
    if (!send_num(so, doc.data_len, ' ')) return 0;
    if (!send_bytes(so, doc.data, doc.data_len)) return 0;
    if (!send_bytes(so, "\n", 1)) return 0;
  }
  if (cx != c) {
    fprintf(stderr, "occurrence count did not match (before: %ld after: %ld)\n",
...
/**
   ...
   その処理, を繰り返す.
  */
static void server_process_connection(int so, server_t * sv) {
...
static cmdline_options_t parse_args(int argc, char ** argv) {
  ...
  while (1) {
    int c = getopt(argc, argv, "l:p:q:h");
    if (c == -1) break;
    switch (c) {
    case 'l':
      my_free(opt.log);
